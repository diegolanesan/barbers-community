...
const Recovery = ()=>{
    ...
    const token = JSON.parse(localStorage.getItem("tokenBarberRecovery"));
    ...
    <div className="recoveryContainer">
        ...
        <div className="conteinerForm">
            {
                fase.fase === 0 ? (
                <form action="" className="recoveryForm" onChange={handleChangeCero} onSubmit={handleSubmitCero}>
                    <label htmlFor="">
                        Enter your email
                    </label>
                ...
                loading ? (<h1 style={{color:"white", fontSize:"20px", fontWeight:"500"}}>LOADING...</h1>):
                err ? (<h1 style={{color:"white", fontSize:"20px", fontWeight:"500"}}>{err.toUpperCase()}</h1>):
                fase.fase === 1 ? (
                    <>
                     <h1 style={{color:"white", fontSize:"20px", fontWeight:"500"}}>Se envio un mail para confirmar</h1>
    
                    {
                        localStorage.setItem("tokenBarberRecovery",JSON.stringify(resp))
                    }
                    </>
                ):
                fase.fase === 2 ? (
                    <form action="" className="recoveryForm" onChange={handleChangeDos} onSubmit={handleSubmitDos} onKeyUp={handleKeyUpDos}>
                    <div>
                    ...
                    {
                        form.error ? (<p style={{color:"red"}}>{form.error}</p>):(<button>SEND</button>)
                    }
                </form>
                ):("")
            }
